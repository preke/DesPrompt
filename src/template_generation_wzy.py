    ...
    def _get_templates(self):
        ...
        for i in tqdm(range(self.max_length - 2)):
            ...
            for item in current_output:
                print('current beam search number', bm_count)
                bm_count += 1
                ...
                for t in range(turn):
                    ...
                    with torch.no_grad():
                        tmp = self.model(input_ids[start:end], attention_mask=attention_mask[start:end], decoder_input_ids=decoder_input_ids.to(input_ids.device)[start:end])[0]
                        

                        if mid_result is None:
                            mid_result = tmp.cpu().sum(0)
                        else:
                            mid_result += tmp.cpu().sum(0)
                        print(mid_result.shape)
                        
#                         aggr_output.append(tmp.cpu())
#                         print('DEL...tmp')
#                         del tmp
#                         gc.collect()
...
#                 # Gather results across all input sentences, and sort generated tokens by log likelihood
                ...
                aggr_output = mid_result / input_ids.size(0)
